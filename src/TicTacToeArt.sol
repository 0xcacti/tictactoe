...
library TicTacToeArt {
        ...
        returns (string memory)
    {
...

        {
            ...
            uint256 winner = gameBoard.getWiunner();
            if (winner == 3) {
                result = "Draw";
            } else if (winner == 1) {
    ...
    function getImageAndAtributes(uint256 _board, uint256 _seed) internal pure returns (string memory, string memory) {
        string memory attributes =
            string(abi.encodePacked('{"trait_type":"Dimension","value":"', unicode" × ", _numSquares.toString(), '"}'));
